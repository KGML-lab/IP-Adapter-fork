...
# CKPT_DIR   = "/scratch/bio_diffusion/ip-adapter_runs/bioclip/taxabind_location_context4_concat/checkpoint-28000"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert checkpoint to bin/safetensors format.")
    parser.add_argument('--ckpt_dir', type=str, default=None, help='Path to checkpoint dir (containing model.safetensors)')
    parser.add_argument('--base_model', type=str, default="runwayml/stable-diffusion-v1-5", help='Base model name')
    parser.add_argument('--base_ckpt_root', type=str, default="/scratch/bio_diffusion/ip-adapter_runs/bioclip/taxabind_location_context4_concat", help='Root dir to search for latest checkpoint if --ckpt_dir not given')
    ...
    if args.ckpt_dir:
        ...
        CKPT_DIR = find_latest_checkpoint(args.base_ckpt_root)
        print(f"Auto-selected latest checkpoint: {CKPT_DIR}")
    IN_SFT = os.path.join(CKPT_DIR, "model.safetensors")
    OUT_BIN = os.path.join(CKPT_DIR, "ip_adapter.bin")
    OUT_SFT = os.path.join(CKPT_DIR, "ip_adapter.safetensors")
